...
void TrackerCore::generateGroundTruth() {
	...
	for (uint i = 0; i < num_frames; i++) {
		...
		for (uint j = 0; j < this->num_trackers; j++) {
			...
			json["frames"][i]["positions"][j]["x_pos"] =
				trackers[j].positions[i].x;
			json["frames"][i]["positions"][j]["y_pos"] =
				trackers[j].positions[i].y;
		}
	}
		...
		for (uint j = 0; j < image.rows; j++)
			for (uint color = 0; color < 3; color++) {
				if (image.at<Vec3b>(i, j)[color] != 0)
					frames.at<Vec3b>(this->y_position + i,
									 this->x_position + j)[color] =
...
void Tracker::createMat() {
	...
	circle(this->image, Point(floor(this->size_x / 2), floor(this->size_y / 2)),
		   floor(this->size_x / 2) - 1, Scalar(red, blue, green), -1, 8);
	...
	for (uint i = 0; i < io::configs.num_features; i++) {
		...
		circle(this->image, Point(floor(this->size_x / 2) + x_variation,
								  floor(this->size_y / 2) + y_variation),
			   ceil(this->size_x / 4), color, -1, 8);
	}
}
